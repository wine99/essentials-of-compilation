...
(define (expose-exp e)
  (match e
    [(HasType (Prim 'vector es) type)
     (expose-exp-vec es type #f)]
    [(HasType (Closure arity funref-and-fvs) type)
     (expose-exp-vec funref-and-fvs type arity)]
    [(Let x e body)
...
(define (expose-exp-vec es type arity)
  ...
  (define alloc-and-init
    (make-alloc-and-init v len type vars exposed-es arity))
...
(define (make-alloc-and-init vec-sym len vec-type vars exposed-es arity)
  (Begin
    (list (If (Prim '< (list (Prim '+ (list (GlobalValue 'free_ptr)
              ...
              (Void)
              (Collect (* 8 (add1 len)))))
    (Let vec-sym (if arity
                     (AllocateClosure len vec-type arity)
                     (Allocate len vec-type))
         (if (= len 0)
             (Var vec-sym)
